...
class Template():
    ...
    def patter_creator(self):
        # display
        self.view = ""
        self.display = ctk.CTkLabel(master=self.root, text="", height=80, font=("Arial", 16))
    ...
    def send(self, value):
        self.view += value
        self.display.configure(text=self.view)

    def button_create(self, text, row, col, command):
        self.btn = ctk.CTkButton(self.calc, text=text, command=command, font=("Arial", 16), height=60)
        ...
        if (self.view[0].isdecimal() == False and self.view[0] != "-") or self.view[-1].isdecimal() == False:
            ...
            for index, object in enumerate(list2[1:]):
                if object == "+":
                    ...
                    pass
            print(solution)
            self.display.configure(text=str(solution))
...
class Functions(Template):
    def __init__(self):
        ...
        self.title = ctk.CTkLabel(master=self.root, text="Input function formula here:", height=80, font=("Arial", 16))
        self.title.pack()

        # formula inputs
        self.input_formula = ctk.CTkEntry(self.root, font=("Arial", 16), justify='center')
        ...
        # range inputs
        self.range_frame = tk.Frame(self.root, bg=MAIN_COLOR)
        self.range_frame.columnconfigure(0, weight=1)
        self.range_frame.columnconfigure(1, weight=1)

        self.range_left_label = ctk.CTkLabel(master=self.range_frame, text="Left part of input range", height=80, font=("Arial", 16))
        self.range_left_label.grid(row=0, column=0, sticky=tk.W + tk.E)
        self.range_left = ctk.CTkEntry(self.range_frame, font=("Arial", 16), justify='center')
        self.range_left.grid(row=1, column=0, sticky=tk.W + tk.E)

        self.range_right_label = ctk.CTkLabel(master=self.range_frame, text="Right part of input range", height=80, font=("Arial", 16))
        self.range_right_label.grid(row=0, column=1, sticky=tk.W + tk.E)
        self.range_right = ctk.CTkEntry(self.range_frame, font=("Arial", 16), justify='center')
        ...
        self.btn = ctk.CTkButton(self.root, text="plot", command=self.make_function)
